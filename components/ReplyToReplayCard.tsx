...
import useReplyToReply from "@/hooks/useReplyToReply";
...
const ReplyToReplayCard: React.FC<replyCardProps> = ({ replyId }) => {
  const { data: replies } = useReplyToReply(replyId)
  ...
  console.log('state.rplId',state.rplId)
    ...
    <>
      {replies?.length !== 0
        ? replies?.map((rpl: any) => {
            ...
            return (
               <div key={rpl.id} className="ml-[50px] pr-4 mb-10 md:ml-[90px]">
                ...
                <div className="my-1 md:ml-[70px]">
                    <p className="text-venetian_red font-bold float-left mr-2 text-xs md:text-[15px]">@{rpl.replyToinTo}</p>
                    <div className="font-normal text-gra text-[13px] text-justify pr-4 md:text-[15px]">{rpl.replyContent}</div>
                </div>
...
export default ReplyToReplayCard;
